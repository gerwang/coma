...
facedata = FaceData(nVal=100, train_file=opt.data + '/train.npy',
                    test_file=opt.data + '/test.npy', reference_mesh_file=reference_mesh_file, pca_n_comp=opt.nz,
                    fitpca=True)
nv = facedata.n_vertex * 3
nTest = facedata.vertices_test.shape[0]

cnn_outputs = np.load(opt.cnn)
pca_outputs = facedata.pca.inverse_transform(facedata.pca.transform(
    np.reshape(facedata.vertices_test, (facedata.vertices_test.shape[0], facedata.n_vertex * 3))))

train_vertices_orig = facedata.vertices_train * facedata.std + facedata.mean
test_vertices_orig = facedata.vertices_test * facedata.std + facedata.mean
pca_orig = PCA(n_components=facedata.pca.n_components)
pca_orig.fit(train_vertices_orig.reshape(train_vertices_orig.shape[0], -1))
pca_outputs_orig = pca_orig.inverse_transform(
    pca_orig.transform(test_vertices_orig.reshape(test_vertices_orig.shape[0], -1)))
...
pca_vertices_orig = pca_outputs_orig.reshape(pca_outputs_orig.shape[0], facedata.n_vertex, 3)

mm_constant = 100  # fixme
cnn_vertices = cnn_vertices * mm_constant
pca_vertices = pca_vertices * mm_constant
test_vertices = test_vertices * mm_constant
pca_vertices_orig *= mm_constant

save_error_plt(cnn=cnn_vertices, pca=pca_vertices, orig=pca_vertices_orig, test=test_vertices,
...
orig_err = np.mean(np.sqrt(np.sum((pca_vertices_orig - test_vertices) ** 2, axis=2)))
...
orig_std = np.std(np.sqrt(np.sum((pca_vertices_orig - test_vertices) ** 2, axis=2)))
...
orig_med = np.median(np.sqrt(np.sum((pca_vertices_orig - test_vertices) ** 2, axis=2)))

print('{} PCA Error: {:.3f}+{:.3f} | {:.3f} CoMA Error: {:.3f}+{:.3f} | {:.3f} PCA_Orig Error: {:.3f}+{:.3f} | {:.3f}'
      .format(os.path.basename(opt.data), pca_err, pca_std, pca_med, cnn_err, cnn_std, cnn_med,
              orig_err, orig_std, orig_med))
